...
### 도메인
- [x] Review 상태를 추가 및 상태별 태스크의 특징을 적용
...
- [x] 태스크 상태 전이 규칙 적용 (허용된 상태로만 전이 가능)
  ...
  - Done -> To Do
- [x] 태스크 도메인의 상태 전이 및 제약 조건(담당자, 삭제) 테스트 코드 작성

### UI
- [x] 태스크 카드를 클릭하여 수정/삭제하는 기능 추가
- [x] 도메인 규칙 위반 시 스낵바 노출 기능 구현
  - [x] Review 또는 Done 상태에서 태스크 삭제 시도시
  - [x] 불가능한 상태 전이 시도시
  - [x] 담당자 없이 To Do -> In Progress 전이 시도 시
- [x] 정상 동작 스낵바 노출 기능 구현 (수정 완료, 삭제 완료 시)
- [x] UI 인터랙션 및 상태 변경 흐름 테스트 작성
  - 카드 클릭 시 수정 모드 진입 여부 확인
  - 도메인 예외 발생 시 적절한 스낵바 메시지가 노출되는지 검증 (Mock Data 활용)

### 리팩토링
- [x] strings.xml 적용 못 한 부분 적용하기
- [x] 태스크 수정을 했을 때, 태스크의 순서 보장하기
